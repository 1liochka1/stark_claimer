...
from info.starknet_info import ARGENT_CLASS_HASH, ARGENT_OLD_CLASH_HASH
...
async def create_call_data(account_class_hash, public_key):
    return [public_key, 0] if account_class_hash in (ARGENT_CLASS_HASH, ARGENT_OLD_CLASH_HASH) else [public_key]
...
async def create_constructor_call_data(account_class_hash, public_key):
    ...
    return [
        ...
        selector.get_selector_from_name(
            "initialize" if account_class_hash == ARGENT_OLD_CLASH_HASH else "initializer"),
